...
class State:
    ...
    def __init__(self, team_a, team_b):
        ...
        self.summarizer = pipeline(PIPELINE_SUMMARIZE)
    ...
    def get_sentiment_for_message(self, message):
        return self.sentiment_analyzer(message)[0]
...
if __name__ == '__main__':
    ...
    state.add_message(TEAM_BLUE,
                      'Liverpool been getting exposed at the back on counters, full backs are far too high and its just VVD and Gomez to have to rush back. #teamBlue')
    ...
    state.add_message(TEAM_BLUE,
                      'who else have difficulty differentiate the two team due to their colors?!!! FML. #teamBlue')
    state.add_message(TEAM_BLUE,
                      'funny how Man City thought they beat Liverpool because they were the better team not because Liverpool dgaf anymore. #teamBlue')
    ...
    state.add_message(TEAM_BLUE,
                      'Dont worry, Nottingham Forest will sneak into the Premier League, and represent the British Midland region property. #teamBlue')
    state.add_message(TEAM_BLUE,
    ...
    state.add_message(TEAM_RED,
                      'How is it that PL games are continuing when Liverpool was named the champions? Someone explain please! #teamRed')
    ...
    state.add_message(TEAM_RED,
                      'NBC WILL PRAISE A WHITE CAPTAIN “Henderson” giving him achievement of black player like MANE, GIGI, Alexander, VIRGIL..while they ignored “Kompany” during Mancity days just praising aguero then Kevin De Bruyen. #teamRed')
    state.add_message(TEAM_RED, 'A Win For Liverpool. #teamRed')
    state.add_message(TEAM_RED,
                      'How does a religiously observant person like Mo Salah square his faith with kneeling before games? #teamRed')
    state.add_message(TEAM_RED, 'this fake fan and stadium noise is terrible. #teamRed')

    print('Blue Team Summary: ', state.get_summary(TEAM_BLUE))
    print('Red Team Summary: ', state.get_summary(TEAM_RED))

    print('Blue Team Avg Sentiment Intensity: ', state.get_sentiment(TEAM_BLUE))
    print('Red Team Avg Sentiment Intensity: ', state.get_sentiment(TEAM_RED))
